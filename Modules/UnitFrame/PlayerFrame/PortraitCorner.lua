...
function PlayerFrame:CreatePortraitCorner(scale)
	...
	local function UpdateCorner(isCombat)
		if isCombat then
			texture:SetTexture(Textures.GetTexturePath("UnitFrame\\PlayerFrame\\PlayerFrame-Portrait-Corner-Combat.blp"))
		else
			texture:SetTexture(Textures.GetTexturePath("UnitFrame\\PlayerFrame\\PlayerFrame-Portrait-Corner.blp"))
		end
	end
